          ...
          if ( strcmp( GDALGetDriverShortName( GDALGetDatasetDriver( mGdalDataset ) ), "netCDF" ) == 0 || strcmp( GDALGetDriverShortName( GDALGetDatasetDriver( mGdalDataset ) ), "GTiff" ) == 0 ) {
              ...
              if (GDALmetadata) {
                  ...
                  for (QStringList::const_iterator i = metadata.constBegin();
                      ...
                      QString val(*i);
                      if (!val.startsWith(QLatin1String("NETCDF_DIM_EXTRA")) &&
                          !val.startsWith(QLatin1String("GTIFF_DIM_EXTRA")) && !val.contains(QLatin1String("#units=")))
                          continue;
                      QStringList values = val.split('=');
                      val = values.at(1);
                      if (values.at(0) == QLatin1String("NETCDF_DIM_EXTRA") ||
                          values.at(0) == QLatin1String("GTIFF_DIM_EXTRA")) {
                  ...
                  if (!dimExtraValues.isEmpty()) {
                      ...
                      if (GDALmetadata) {
                          ...
                          for (QStringList::const_iterator i = metadata.constBegin();
                              ...
                              if (!val.startsWith(QLatin1String("NETCDF_DIM_")) &&
                                  !val.startsWith(QLatin1String("GTIFF_DIM_")))
                                  continue;
                              QStringList values = val.split('=');
                              for (QStringList::const_iterator j = dimExtraValues.constBegin();
                                  ...
                                  if (values.at(0) != "NETCDF_DIM_" + dim && values.at(0) != "GTIFF_DIM_" + dim)
                                      continue;
                                  if (unitsMap.contains(dim) && !unitsMap[dim].isEmpty() &&
                                      unitsMap[dim] != QLatin1String("none"))
                      ...
                      if (!bandNameValues.isEmpty())
                          return tr("Band") +
                                 QStringLiteral(" %1 / %2").arg(bandNumber, 1 + (int) std::log10((float) bandCount()),
                                                                10, QChar('0')).arg(
                                         bandNameValues.join(QStringLiteral(" / ")));
                  }
              }
          }
          break;
